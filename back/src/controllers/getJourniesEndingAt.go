...
func GetJourniesEndingAt(c *fiber.Ctx) error {
	id := c.Params("id")
	month := c.Query("month")
	page := c.Query("page")
	show := c.Query("show")
	...
	count, error := db.NewSelect().
		...
		Where("return_station_id = ? AND DATE_PART('month', departure) = ?", id, month).
		Count(ctx)
	...
	q, err := libs.PaginationQuery(db.NewSelect().
		Model(&results).
		Where("return_station_id = ? AND DATE_PART('month', departure) = ?", id, month), &f)
	if err != nil {
		return c.Status(http.StatusInternalServerError).
